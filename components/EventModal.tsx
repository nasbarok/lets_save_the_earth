    ...
    <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 transition-opacity duration-300">
      <div className="bg-slate-800 rounded-lg max-w-lg w-full mx-4 shadow-2xl ring-1 ring-slate-600 animate-fade-in-up relative">
        <div className="aspect-video w-full bg-slate-700/50 rounded-t-lg flex items-center justify-center relative">
            {event.imageUrl ? (
                <img src={event.imageUrl} alt={event.title} className="w-full h-full object-cover rounded-t-lg"/>
            ) : (
        ...
        <div className="p-6">
            <h2 className="text-2xl font-bold mb-3 text-green-400">{event.title}</h2>
                        ...
                        {event.choices.map((choice, index) => {
                            ...
                            return (
                                 <button
                                    ...
                                    disabled={!canAfford}
                                    className="w-full text-left p-3 bg-slate-700 rounded-lg hover:bg-slate-600 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-green-500"
                                >
