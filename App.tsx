...
import type { GameState, Action, ActiveGameEvent, Country, ActiveAction, SaveSlotInfo, EventChoice, GameEventEffects, Pollutants, Conflict, Language, GeminiNuclearEvent, GeminiIntroNarration, Tech } from './types';
...
import { createInitialState, GAME_SPEED_MS, MIN_EVENT_TICKS, MAX_EVENT_TICKS, CO2_STABILITY_INDEX, WARMING_RATE_FACTOR, CONFLICT_CHECK_TICKS, EXTINCTION_CHECK_TICKS, POPULATION_GAMEOVER_THRESHOLD, INTERSTITIAL_AD_TICKS } from './constants/state';
import { CONFLICT_INSTABILITY_THRESHOLD, CONFLICT_INTENSITY_EFFECTS } from './constants/conflicts';
import { generateRandomEvent, fetchRealTimeData, generateConflictEvent, generateExtinctSpecies, generateNuclearEvent, generateIntroNarration } from './services/geminiService';
...
const Game: React.FC = () => {
  ...
  const addToLog = useCallback((message: string, isKey = false, replacements?: { [key: string]: string | number }) => {
    setGameState(prev => {
        setEventLog(logPrev => {
  ...
  const handleCloseEvent = () => {
    setGameState(prev => {
        if (prev.activeEvent?.isConflictDeclaration) {
            addToLog('logs.warErupted', true, { conflictName: prev.activeEvent.conflictDetails!.name, country1Name: t(prev.countries.find(c=>c.id === prev.activeEvent!.conflictDetails!.involvedCountryIds[0])!.name), country2Name: t(prev.countries.find(c=>c.id === prev.activeEvent!.conflictDetails!.involvedCountryIds[1])!.name) });
            return {
                ...prev,
                activeEvent: null,
    ...
    const gameTick = setTimeout(() => {
      setGameState(prev => {
        ...
        completedActions.forEach(completedAction => {
            ...
            addToLog('logs.projectCompleted', true, { actionName: t(actionDetails.name), countryName: t(country.name) });
            ...
            actionDetails.effects.forEach(effect => {
                ...
                switch (effect.type) {
                    ...
                    case PollutantType.Plastic:
                    case PollutantType.Deforestation:
                        newCountries = newCountries.map(c => c.id === completedAction.countryId ? { ...c, pollutants: { ...c.pollutants, [effect.type]: Math.max(0, c.pollutants[effect.type] + effectValue) } } : c);
                        break;
                    case 'publicSupport':
                        newPublicSupport = Math.min(100, newPublicSupport + effectValue);
                        break;
                    case 'energyRenewable':
                        newCountries = newCountries.map(c => c.id === completedAction.countryId ? { ...c, energy: { ...c.energy, production: {...c.energy.production, renewable: c.energy.production.renewable + effectValue }}} : c);
                        break;
                    case 'energyNuclear':
                        newCountries = newCountries.map(c => c.id === completedAction.countryId ? { ...c, energy: { ...c.energy, production: {...c.energy.production, nuclear: c.energy.production.nuclear + effectValue }}} : c);
                        break;
                    case 'energyThermal':
                        newCountries = newCountries.map(c => c.id === completedAction.countryId ? { ...c, energy: { ...c.energy, production: {...c.energy.production, thermal: Math.max(0, c.energy.production.thermal + effectValue) }}} : c);
                        break;
                }
            });
        });
        ...
        newCountries = newCountries.map(c => {
            ...
            return { ...c, gdp: newGdp, population: newPopulation, pollutants: newPollutants, healthIndex: newHealthIndex, instability: newInstability };
        });
  ...
  useEffect(() => {
    const triggerEvent = async () => {
        ...
        if (eventData) {
            ...
            const imageUrl = IMAGES[category as keyof typeof IMAGES] || IMAGES.DEFAULT;
            setGameState(prev => ({
                ...prev,
                activeEvent: {
                    id: `event-${Date.now()}`,
        ...
        setGameState(prev => ({ ...prev, ticksUntilNextEvent: Math.floor(Math.random() * (MAX_EVENT_TICKS - MIN_EVENT_TICKS + 1)) + MIN_EVENT_TICKS }));
  ...
  useEffect(() => {
    gameState.countries.forEach(country => {
        ...
        if (crisisLevel > country.healthCrisisLevel) {
            const { title, description, baseEffects, choices } = createHealthCrisisAlert(t(country.name), crisisLevel, t);
            
            let imageUrl = IMAGES.DEFAULT;
            if (crisisLevel === 1) imageUrl = IMAGES.HEALTH_WARNING;
            if (crisisLevel === 2) imageUrl = IMAGES.HEALTH_CRISIS;
            if (crisisLevel === 3) imageUrl = IMAGES.HEALTH_EMERGENCY;
            if (crisisLevel === 4) imageUrl = IMAGES.HEALTH_COLLAPSE;

            setGameState(prev => ({
                ...prev,
                countries: prev.countries.map(c => c.id === country.id ? { ...c, healthCrisisLevel: crisisLevel } : c),
                activeEvent: {
    ...
    const checkExtinction = async () => {
        ...
        if (Math.random() < extinctionChance) {
            ...
            const causes = [
                ...
                { name: t('cause.deforestation'), weight: gameState.countries.reduce((s,c) => s + c.pollutants.deforestation, 0) / gameState.countries.length },
            ...
            if (speciesData) {
                addToLog('logs.extinctionSuffix', true, { speciesName: speciesData.speciesName, cause: dominantCause });
            }
            setIsGenerating(false);
        }
  ...
  useEffect(() => {
    if (!isPaused && gameHasStarted) {
      ...
      if (threatIncrease > 0) {
        setGameState(prev => ({ ...prev, nuclearThreatLevel: Math.min(100, prev.nuclearThreatLevel + threatIncrease) }));
      }
    }
          ...
          setGameState(prev => {
            const newCountries = prev.countries.map(c => {
              if (eventData.targets.includes(c.id)) {
                ...
                return { ...c, population: Math.max(0, c.population - popLoss), healthIndex: Math.max(0, c.healthIndex - healthLoss) };
              }
              return c;
            });
  ...
  return (
    <div className="w-full max-w-full mx-auto p-1 sm:p-2 font-sans relative overflow-x-hidden">
      ...
      {gameState.activeEvent && <EventModal event={gameState.activeEvent} timer={gameState.eventTimer} ecoPoints={gameState.ecoPoints} onChoice={handleEventChoice} onAcknowledge={handleCloseEvent}/>}
      ...
      <main className="flex flex-col lg:grid lg:grid-cols-3 gap-2 mt-2 lg:flex-grow lg:min-h-0">
        <div className="lg:col-span-2 flex flex-col gap-4 lg:min-h-0">
          ...
          <div className="lg:flex-grow lg:min-h-0">
            <WorldMap countries={gameState.countries} activeConflicts={gameState.activeConflicts} selectedCountryId={gameState.selectedCountryId} onSelectCountry={handleCountrySelect} />
          </div>
        </div>
